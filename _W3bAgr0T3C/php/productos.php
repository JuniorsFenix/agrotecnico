									...
									while ($rax = mysqli_fetch_assoc($result)) : ?>
										<li>
											<a style="display:block;" href='<?php echo "$home/productos/marca/{$rax["url"]}" ?>'>
												...
												<?php
												if ($rax["imagen"] == 'dontus.jpeg') {
													echo "<img style='height:110px' src='$home/fotos/tienda/marcas/{$rax["imagen"]}' alt='{$rax["nombre"]}' />";
												} else {
													echo "<img src='$home/fotos/tienda/marcas/{$rax["imagen"]}' alt='{$rax["nombre"]}' />";
												}
												?>
								...
								while ($rax = mysqli_fetch_assoc($result)) : ?>
									<div class="listado col-sm-6 col-md-5 col-xl-4">
										<div class="content-product">
											...
											<a class="imagenBlog" href='<?php echo "$home/productos/categoria/{$rax["url"]}" ?>'>
											...
											<a href='<?php echo "$home/productos/categoria/{$rax["url"]}" ?>' class='link'>Ver más</a>
										</div>
									</div>
								<?php endwhile; ?>
							</div>
