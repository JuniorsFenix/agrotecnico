<?php
	...
	if (isset ($_POST["clave2"]) && !empty($_POST["clave2"])){
		$sql="SELECT * FROM tblusuarios_externos WHERE idusuario={$_SESSION['usuario']['idusuario']}";
		$ra = mysqli_query($nConexion,$sql);
		$rax = mysqli_fetch_assoc($ra);
		
    	$clave1 = hash("sha256",$_POST["clave1"]);
    
		if($rax["clave"]!==$clave1){
			$mensaje = "<div class='error'><h4>No se pudo cambiar la contraseña</h4><p>Contraseña anterior incorrecta</p></div>";
		}
		else{
    	$clave2 = hash("sha256",$_POST["clave2"]);
		$sql="UPDATE tblusuarios_externos SET clave='$clave2' WHERE idusuario={$_SESSION['usuario']['idusuario']}";
		mysqli_query($nConexion,$sql);
			$mensaje = "<div class='mensaje'><h4>Contraseña guardada con éxito</h4></div>";
		}
	}
...
<head>
    ...
    <title>Cambiar contraseña - <?php echo $sitioCfg["titulo"]; ?></title>
				...
				<div class="col-sm-3">
					...
					<h2>Configuración</h2>
						<ul>
							...
							<li><a class="activo" href="/mi-cuenta/cambiar-clave">Cambio de clave</a></li>
				...
				<div class="col-sm-9">
					...
					<div class="row review">
						...
						<div class="col-md-12">
					   		...
					   		<input type="password" class="form-fullwidth" name="clave1" required>
						</div>
						<div class="col-md-12">
					   		<label>Contraseña nueva*</label>
					   		<input type="password" class="form-fullwidth" name="clave2" required>
						</div>
