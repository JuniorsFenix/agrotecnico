...
// https://agrotecnico.com.co/php/carroCoor.php
  ...
  $espec = mysqli_query($nConexion,"SELECT * FROM tblti_espec WHERE idproducto={$rxProducto["idproducto"]} order by nombre asc");
  $espec2 = mysqli_query($nConexion,"SELECT * FROM tblti_espec WHERE idproducto={$rxProducto["idproducto"]} order by nombre asc");
		...
		<div class="row">
			<div class="col-md-6" style="display: flex; flex-direction: column; justify-content: flex-start;">
				<div class="info-producto">
                    <?php if($rxProducto["referencia_activo"]==1){?>
                      <b>Referencia:</b> <?=$rxProducto["referencia"];?>				
                    <?php } ?>
                    <h2><?php echo $rxProducto["nombre"];?></h2>
                    <?php if($rxProducto["inventario"]==0 ){?>
                      <h3 class="clear" style="color:red;font-weight: bold;">No disponible</h3>
                    <?php } else{ ?>
                      <h3 class="clear" style="color:green;font-weight: bold;">Stock disponible</h3>
                    <?php } ?>
                      ...
                      $(function() {
                        ...
                        $('#rating').barrating({
                          ...
                          onSelect: function(value, text) {
                            $.post(
                              '/php/calificar.php', {
                              rating: value, idproduct: idproduct
                            },
                            function() {
                ...
                <div style="text-align:left"><a class="ampliar" href="#">Ver información importante</a></div>
			</div>
			...
			<div class="col-md-6">
              ...
              <div>
                ...
                <?php }?>
				<div class="info-producto2 align-items-end justify-content-between">
            <div class="precio">
                ...
                <span>
                  COP <?php
                      if(DIASINIVA AND $rxProducto["diasiniva"]) {
                          echo number_format( ($rxProducto["precio"]/1.19), 0, ',', '.' );
                        }else{
                          echo number_format( ($rxProducto["precio"]), 0, ',', '.' ); 
                        }
                      ?>
                </span>
              <?php } ?>					  
            </div>
            <?php if($rxProducto["activo"]==1 AND $rxProducto["inventario"]>0 ){?>
            <div class="botonCarro">
                <div class="productPriceWrapRight cesta">
                    <span><input type="number" id="cantidad" name="cantidad" min="1" max="<?=$rxProducto["inventario"]?>" value="1" ></span><a href="<?php echo $home; ?>/php/inc/functions.php?action=addToBasket&productID=<?=$rxProducto["idproducto"];?>" id="featuredProduct_<?=$rxProducto["idproducto"];?>" class="1" onClick="return false;">
...
echo $sql;
    ...
    <?php } ?>
		...
		<a href="javascript:window.history.back();" class="volver">Volver</a>
	</div>
</section>    
