<?php
  ...
  $sql="SELECT  c.carro as Pedido,
     ...
     LEFT JOIN  tblti_cotizaciones f
            ON  c.carro=f.carro    
     LEFT JOIN  tblusuarios_externos u
            ON  c.idusuario=u.idusuario             
         WHERE  c.idusuario!=0 AND c.estado!='CARRO'
      GROUP BY  c.carro  
  ...
  $campos = ["Marcas", "Productos", "# Cotización", "Fecha", "Sede", "Asesor", "Cliente", "Teléfono", "Dirección", "Ciudad", "Precio", "Cantidad", "Estado", "Observaciones", "Encuesta", "Cotización", "Url"];  
  $count = count($campos);
...
<head>
    ...
    <title>Cotizaciones - <?php echo $sitioCfg["titulo"]; ?></title>
		...
		<div class="contenidoGeneral cuenta" style="min-height: 800px">
			<div class="container">
              ...
              <?php if($_SESSION["usuario"]["perfil"] == 1) {
                echo "<li><a class='activo' href='$home/cotizaciones'>Cotizaciones</a></li>";
              } ?>
                ...
                <?php
                  ...
                  <tr>
                    <?php 
                      ...
                      $rs = mysqli_query($nConexion,"SELECT nombre FROM tblti_marcas WHERE idmarca IN (SELECT idmarca FROM tblti_productos WHERE idproducto IN (SELECT idproducto FROM tblti_carro_detalle WHERE carro={$row["Pedido"]}))");
                      while($rax=mysqli_fetch_assoc($rs)):
                        $marcas.="-{$rax["nombre"]}<br>";
                      endwhile;
                      $marcas = rtrim($marcas, '<br>');

                      $rs = mysqli_query($nConexion,"SELECT item FROM tblti_carro_detalle WHERE carro={$row["Pedido"]}");
                      while($rax=mysqli_fetch_assoc($rs)):
                        $productos.="-{$rax["item"]}<br>";
                      endwhile;
                      $productos = rtrim($productos, '<br>');
                      ...
                      switch ($campo){
                        case "Pedido":
                          $id = str_replace("Cotizacion-AT","",$row["Cotización"]);
                        ...
                        case "Cotización":   
                          ...
                          echo "<td style='text-align:right;'><a target='_blank' href='/fotos/tienda/cotizaciones/{$valor}'>$factura</a></td><td>https://www.agrotecnico.com.co/fotos/tienda/cotizaciones/{$valor}</td>";  
                          break;                          
                        default:
                          echo "<td>$valor</td>";  
                      }                     
    ...
    $(document).ready(function() {
      ...
      buttons:[{
        ...
        customize: function( xlsx ) {
          ...
          $('row c[r^="B"]', sheet).slice(2).attr( 's', '55' );
          $('row c[r^="C"]', sheet).slice(2).attr( 's', '55' );
          ...
          // if cell starts with http
            if ( $('is t', this).text().indexOf("http") === 0 ) {
              ...
              $(this).attr('t', 'str');
              //append the formula
              $(this).append('<f>' + 'HYPERLINK("'+$('is t', this).text()+'","Descargar PDF")'+ '</f>');
