...
function myerr()
{
  $e = mysqli_error($nConexion);
  mysqli_free_result( mysqli_query($nConexion, 'ROLLBACK' ) );
  err( $e );
}
...
if( $_POST['server'] == 'mysql' ) {
  ...
  if( $_POST['port'] )
    $host .= ':'.$_POST['port'];
  $conn = mysqli_connect($host,$_POST['user'],$_POST['password']) or myerr();
  if( $_POST['dbname'] != '' )
    mysqli_select_db( $_POST['dbname'] ) or myerr();
  if( array_key_exists('charset',$_POST) && $_POST['charset'] != '' )
    mysqli_query($nConexion, 'SET NAMES \'' . $_POST['charset'] . '\'' ) or myerr();
  $result = FALSE;
  mysqli_free_result( mysqli_query($nConexion, 'BEGIN' ) );
  for( $rn = 1; $rn < 1000; ++$rn ) {
    ...
    $req = $_POST['r'.$rn];
    if( $req == 'connect' ) {
      dump( mysqli_get_server_info() );
      dump( mysqli_get_client_info() );
      dump( mysqli_get_proto_info() );
      dump( mysqli_get_host_info() );
    } else {
      $result = mysqli_query($nConexion,$req) or myerr();
      if( $result === TRUE ) {
        dump( 0 );
        dump( mysqli_affected_rows() );
      } else {
  ...
  mysqli_free_result( mysqli_query($nConexion, $commit ? 'COMMIT' : 'ROLLBACK' ) );
