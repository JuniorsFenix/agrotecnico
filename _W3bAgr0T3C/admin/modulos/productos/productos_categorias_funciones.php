...
?>
    ...
    <form action="productos_categorias.php?Accion=Guardar" method="post" enctype="multipart/form-data">
      ...
      <table width="100%">
        ...
        <tr>
        		<td class="tituloNombres">Nombre Alfabeto:</td>
        ...
        <tr>
          <td colspan="2" class="nuevo">
            ...
            <a href="productos_categorias_listar.php"><img src="../../image/cancelar.gif" border="0" alt="Cancelar y Regresar."></a>
          </td>
        </tr>
      </table>
</form>
  ...
  function CategoriasGuardar( $nIdPadreActual,$nId,$nIdPadre,$cCategoria,$cDescripcion,$cImagen )
  {
    ...
    if ( $nId <= 0 ) // Nuevo Registro
    {
      ...
      //mysqli_query($nConexion,"INSERT INTO tblcategoriasproductos ( idpadre,categoria,descripcion,imagen ) VALUES ( '$nIdPadre','$cCategoria','$cDescripcion','$cImagen' )");
			if ( $nIdPadre == 0 )
			{
				...
				$rsContador						= mysqli_query($nConexion,"SELECT MAX( CAST(idcategoria AS UNSIGNED) ) AS contador FROM tblcategoriasproductos WHERE (idpadre = '$idNuevoPadre') AND (idciudad = $IdCiudad)");
				...
				mysqli_query($nConexion,"INSERT INTO tblcategoriasproductos ( idcategoria,idpadre,categoria,descripcion,imagen,idciudad ) VALUES ('$NuevoIdCategoria','0', '$cCategoria','$cDescripcion','$cImagen',$IdCiudad)");
				Log_System( "PRODUCTOS CATEGORIAS" , "NUEVO" , "CATEGORIA: " . $cCategoria  );
			} //FIN if ( $nIdPadre == 0 )
			else
			{
				$rsContador = mysqli_query($nConexion,"SELECT COUNT(*)+1 AS contador FROM tblcategoriasproductos WHERE (idpadre = '$nIdPadre') AND (idciudad = $IdCiudad)");
				...
				mysqli_query($nConexion,"INSERT INTO tblcategoriasproductos (idcategoria,idpadre,categoria,descripcion,imagen,idciudad) VALUES ('$cCodCategoria','$nIdPadre','$cCategoria','$cDescripcion','$cImagen',$IdCiudad)");
				Log_System( "PRODUCTOS CATEGORIAS" , "NUEVO" , "CATEGORIA: " . $cCategoria  );
			}
      mysqli_close($nConexion);
      Mensaje( "El registro ha sido almacenado correctamente.", "productos_categorias_listar.php" ) ;
      exit;
    }
    else // Actualizar Registro Existente
    {
			...
			if ( $nIdPadreActual != $nIdPadre )
			{
				...
				$rsContador						= mysqli_query($nConexion,"SELECT COUNT(*) AS contador FROM tblcategoriasproductos WHERE (idpadre = '$idNuevoPadre') AND (idciudad = $IdCiudad)");
				$RegistrosRSContador	= mysqli_fetch_object($rsContador);
				$Contador							= $RegistrosRSContador->contador;
				if ( $Contador == null )
				{
				...
				if ( !empty( $cImagen ) )
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasproductos SET idcategoria = '$NuevoIdCategoria', idpadre = '$idNuevoPadre', categoria = '$cCategoria', descripcion = '$cDescripcion', imagen = '$cImagen' WHERE idcategoria = '$nId' AND (idciudad = $IdCiudad)" );
					Log_System( "PRODUCTOS CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				else
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasproductos SET idcategoria = '$NuevoIdCategoria', idpadre = '$idNuevoPadre', categoria = '$cCategoria', descripcion = '$cDescripcion' WHERE idcategoria = '$nId' AND (idciudad = $IdCiudad)" );
					Log_System( "PRODUCTOS CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				// Actualizo las categorias de los productos que pertenecian al anterior codigo categoria
				mysqli_query($nConexion,"UPDATE tblproductos SET idcategoria = '$NuevoIdCategoria' WHERE (idcategoria =  '$nId') AND (idciudad = $IdCiudad)");
			}
			else // No cambio la categoria actualizo los demas datos
			{
				if ( !empty( $cImagen ) )
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasproductos SET categoria = '$cCategoria', descripcion = '$cDescripcion', imagen = '$cImagen' WHERE idcategoria = '$nId' AND (idciudad = $IdCiudad)" );
					Log_System( "PRODUCTOS CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				else
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasproductos SET categoria = '$cCategoria', descripcion = '$cDescripcion' WHERE idcategoria = '$nId' AND (idciudad = $IdCiudad)" ) or die (mysqli_error($nConexion));
					Log_System( "PRODUCTOS CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
			}
      mysqli_close( $nConexion );
      Mensaje( "El registro ha sido actualizado correctamente.", "productos_categorias_listar.php" ) ;
      exit;
    }
  } // FIN: function 
  ###############################################################################
...
<?
  ...
  function CategoriasEliminar( $nId )
  {
		$IdCiudad = $_SESSION["IdCiudad"];
    $nConexion = Conectar();
		$reg = mysqli_fetch_object( mysqli_query($nConexion,"SELECT categoria FROM tblcategoriasproductos WHERE idcategoria ='$nId' AND (idciudad = $IdCiudad)") );
    mysqli_query($nConexion, "DELETE FROM tblcategoriasproductos WHERE idcategoria ='$nId' AND (idciudad = $IdCiudad)" );
		Log_System( "PRODUCTOS CATEGORIAS" , "ELIMINA" , "CATEGORIA: " . $reg->categoria  );
    mysqli_close( $nConexion );
    Mensaje( "El registro ha sido eliminado correctamente.","productos_categorias_listar.php" );
    exit();
...
<?
  ...
  function CategoriasFormEditar( $nId )
  {
    ...
    $nConexion    = Conectar();
    $Resultado    = mysqli_query($nConexion, "SELECT tblcategoriasproductos.idcategoria, tblcategoriasproductos.idpadre, tblcategoriasproductos.categoria, tblcategoriasproductos.descripcion, tblcategoriasproductos.imagen, COUNT(tblproductos.idproducto) AS tproductos FROM tblcategoriasproductos LEFT OUTER JOIN tblproductos ON tblcategoriasproductos.idcategoria = tblproductos.idcategoria WHERE tblcategoriasproductos.idcategoria = '$nId' AND (tblcategoriasproductos.idciudad = $IdCiudad) GROUP BY tblcategoriasproductos.idcategoria, tblcategoriasproductos.idpadre, tblcategoriasproductos.categoria, tblcategoriasproductos.descripcion, tblcategoriasproductos.imagen" ) ;
    $ResultadoSubCat = mysqli_query($nConexion, "SELECT COUNT(tblcategoriasproductos.idcategoria) AS tsubcategorias FROM tblcategoriasproductos WHERE (idpadre = '$nId') AND (idciudad = $IdCiudad)" );
		mysqli_close( $nConexion ) ;
...
?>
    ...
    <form method="post" action="productos_categorias.php?Accion=Guardar" enctype="multipart/form-data">
      ...
      <table width="100%">
        ...
        <tr>
        		...
        		<td class="contenidoNombres">
						<?
							...
							{
								?><img src="<? echo $cRutaVerImgCategorias_Productos . $Registro["imagen"]; ?>"><?
							}
						?>
						</td>
       		</tr>
        <tr>
          ...
          <?
            $cMSGEliminar = "";
						$cEliminable  = "SI";
            if ( (($Registro[ "tproductos" ] > 0) or ($RegistroSubCat[ "tsubcategorias" ] > 0)) )
            {
							$cEliminable  = "NO";
							if ( $Registro[ "idpadre" ] == 0  )
							{
          ...
          <td class="contenidoNombres"><? echo $RegistroSubCat[ "tsubcategorias" ] . " SubCategor�a(s).<br>" ;?></td>
				...
				<tr>
					<td>&nbsp;</td>
					<td class="contenidoNombres"><? echo $Registro[ "tproductos" ] . " Producto(s).<br>" ?></td>
				</tr>
				<?	
				if ( $cEliminable == "NO" )
				{
        ...
        <tr>
          <td colspan="2"  class="nuevo">
            ...
            <?
              ...
              if ( $cEliminable == "SI" )
              {
                ?><a href="productos_categorias.php?Accion=Eliminar&Id=<? echo $nId ;?>"><img src="../../image/eliminar.gif" border="0" alt="Eliminar Registro." onClick="javascript: return confirm('�Esta seguro que desea eliminar este registro?')"></a><?
              }
						}
            ?>
            <a href="productos_categorias_listar.php"><img src="../../image/cancelar.gif" border="0" alt="Cancelar y Regresar."></a>
