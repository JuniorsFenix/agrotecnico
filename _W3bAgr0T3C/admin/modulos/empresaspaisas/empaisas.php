<?
	...
	include("empaisas_funciones.php");
	include("../../herramientas/upload/uploaderFunction.php");
	include("../../vargenerales.php");
	if (!isset ($_GET["Accion"])) // Si no se envio la accion muestro la lista
	{
	header("Location: empaisas_listar.php");
	}
?>
<html>
	<head>
		<title>Administraci�n de Empresas Paisas</title>
  ...
  */
    switch($_GET["Accion"])
    {
      case  "Adicionar":
        ...
        break;
      case "Editar":
        EmpaisasFormEditar($_GET["Id"]);
        break;
      case "Eliminar":
        EmpaisasEliminar($_GET["Id"]);
        break;
      case "Guardar":
	  // Subo el archivo al servidor, si es correcto almaceno el registro
		...
		//Inicip Para Capturar nombre de la empresa
			
			$nConexion = Conectar();
			$id_nomempre=mysqli_query($nConexion,"select * from tblnombempaisa where id_nombempaisa=$_POST[id_nombempaisa]" ); 
            mysqli_close($nConexion);
			$RegistrosEm=mysqli_fetch_object($id_nomempre);
			$nomempre=$RegistrosEm->nombempaisa;
			
          // Fin de captura nombre de la empresa  
		
        if ( empty($Archivo) )
        {
		EmpaisasGuardar($_POST["txtId"],$nomempre,$_POST["sub"],$_POST["Categoria"],
		$_POST["txtDescripcion"],$_POST["txtPreciomin"],$_POST["txtPreciomax"],$_POST["txtDireccion"],
		$_POST["txtTelefono"],$_POST["txtEmail"],$_POST["txtUrl"],$_POST["txtCiudad"],$_POST["txtPais"],"",$_POST["optPublicar"]);
          exit;
        }
        ...
        # Le damos valor a la variable $size para permitir un tama�o especifico.
        ...
        if(uploader($Campofile,$folder,$size,$tipos))
        {
          ...
          foreach ($uploader_archivos_copiados as $imagen => $detalles)
          {
            ...
            # $imagen que almacena los nombres de las imagenes
     	    ...
     	    # base de datos:
      	    # mysqli_query($nConexion,"Insert into tablmodho (imagen,tipo,size) values 				  
			# (".$imagen.",".$detalles[0].",".$detalles[1].")",$coneccion);
			# mysqli_query($nConexion,"Insert into tblmodho (imagen) values (".$imagen.")",$coneccion);
          }
			EmpaisasGuardar($_POST["txtId"],$nomempre,$_POST["sub"],$_POST["Categoria"],
		$_POST["txtDescripcion"],$_POST["txtPreciomin"],$_POST["txtPreciomax"],$_POST["txtDireccion"],
		$_POST["txtTelefono"],$_POST["txtEmail"],$_POST["txtUrl"],$_POST["txtCiudad"],$_POST["txtPais"],$NomImagen,$_POST["optPublicar"]);
		}
        break;
      default:
        header("Location: empaisas_listar.php");
