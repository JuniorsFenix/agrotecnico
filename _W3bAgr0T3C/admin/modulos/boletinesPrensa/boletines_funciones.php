<?
  ###############################################################################
  # boletines_funciones.php   :  Archivo de funciones modulo boletines
  ...
  function BoletinesFormNuevo()
  {
    $nConexion = Conectar();
?>
    <!-- Formulario Ingreso de Boletines -->
...
<?
  ...
  # Retorno       : Ninguno
  ###############################################################################
  function BoletinesGuardar( $nId,$cNombre,$cDescripcion,$cFuente,$cTags,$cPalabras,$files,$cPublicar )
  {
	include("../../vargenerales.php");
    ...
    {
      mysqli_query($nConexion,"INSERT INTO tblboletinesprensa ( nombre,url,descripcion,fuente,tags,palabras,fechapub,publicar,idciudad ) VALUES ( '$cNombre','$url','$cDescripcion','$cFuente','$tags','$palabras',Now(),'$cPublicar',$IdCiudad )");
      $nId = mysqli_insert_id($nConexion);
			Log_System( "BOLETINES" , "NUEVO" , "TITULAR: " . $cNombre );
			...
			if( isset( $files["txtImagen"]["tmp_name"] ) && $files["txtImagen"]["tmp_name"]!="" ) {
			  ...
			  $image->save($cRutaImgBoletines . $NomImagenG);
			  $image->resizeToWidth(500);
			  $image->save($cRutaImgBoletines . $NomImagenM);
				
				$sql="UPDATE tblboletinesprensa SET imagen = '{$NomImagenG}' where idboletin ='$nId'";

      $r = mysqli_query($nConexion,$sql);

      
      if(!$r){
        Mensaje( "Fallo actualizando imagenes.".mysqli_error($nConexion), "boletines_listar.php" ) ;
        exit;
      }
    ...
    {
        mysqli_query($nConexion, "UPDATE tblboletinesprensa SET nombre = '$cNombre',url = '$url',descripcion = '$cDescripcion',fuente = '$cFuente',tags = '$tags',palabras = '$palabras',publicar='$cPublicar' WHERE idboletin = '$nId'" );
			Log_System( "BOLETINES" , "EDITA" , "TITULAR: " . $cNombre );
  ...
  {
    $nConexion = Conectar();
		$reg = mysqli_fetch_object( mysqli_query($nConexion,"SELECT nombre FROM tblboletinesprensa WHERE idboletin ='$nId'") );
    mysqli_query($nConexion, "DELETE FROM tblboletinesprensa WHERE idboletin ='$nId'" );
		Log_System( "BOLETINES" , "ELIMINA" , "TITULAR: " . $reg->nombre  );
    mysqli_close( $nConexion );
  ...
  function BoletinesFormEditar( $nId )
  {
		include("../../vargenerales.php");
    $nConexion    = Conectar();
    $Resultado    = mysqli_query($nConexion, "SELECT * FROM tblboletinesprensa WHERE idboletin = '$nId'" ) ;

    $Registro     = mysqli_fetch_array( $Resultado );
    $tags=explode(",",$Registro["tags"]);
    $palabras=explode(", ",$Registro["palabras"]);
?>
    <!-- Formulario Edici�n / Eliminaci�n de Boletines -->
    <form method="post" action="boletines.php?Accion=Guardar" enctype="multipart/form-data">
      <input TYPE="hidden" id="txtId" name="txtId" value="<? echo $nId ; ?>">
      <table width="100%">
        <tr>
          <td colspan="4" align="center" class="tituloFormulario"><b>EDITAR BOLET�N</b></td>
        </tr>
        <tr>
          <td class="tituloNombres">Nombre:</td>
    ...
    mysqli_close( $nConexion ) ;?>
        ...
        <tr>
          ...
          <?
            ...
            {
              ?><img src="<? echo $cRutaVerImgBoletines ."m_". $Registro["imagen"]; ?>" width="500"><?
            }
          ?>
          </td>
        <tr>
