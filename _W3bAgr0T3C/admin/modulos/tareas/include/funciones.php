...
function categoriasQuery(){
	global $db;
	$result = $db->query("SELECT * from acciones_categorias");
	if ($result !== false) {
		return $result ;
	} else {
	...
	<nav id="navigation">
		<ul id="main-menu">
			...
			<li class="parent">
				<a href="#">Tareas</a>
				<ul class="sub-menu">
					<li><a href="<?php echo $sitioCfg["carpeta"]; ?>/admin/modulos/tareas/configuracion.php"><i class="icon-wrench"></i> Configuración</a></li>
					<li>
...
function accionesQuery($id='',$todas=''){
	global $db;
	if($id==""){
	...
	if($todas==""){
		$query= "SELECT a.*, at.fecha, at.estado, at.idusuario FROM acciones a JOIN acciones_tareas at on a.id=at.idaccion WHERE at.idtarea=$id";
		return $stmt = $db->query($query);
	}
	$query= "SELECT a.*, at.idusuario FROM acciones a LEFT JOIN acciones_tareas at on a.id=at.idaccion and at.idtarea=$id";
	return $stmt = $db->query($query);
}
...
function checkAcciones($idcategoria){
	global $db;
	$stmt = $db->prepare("SELECT idaccion FROM categorias_acciones WHERE idcategoria=:idcategoria");
	$stmt->bindValue(':idcategoria', $idcategoria, PDO::PARAM_INT);
	$stmt->execute();
	return $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
}

function checkTareas($idtarea){
	global $db;
	$stmt = $db->prepare("SELECT idaccion FROM acciones_tareas WHERE idtarea=:idtarea");
	$stmt->bindValue(':idtarea', $idtarea, PDO::PARAM_INT);
	$stmt->execute();
	return $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
}
...
function guardarTarea($d){
	...
	$stmt = $db->prepare("INSERT INTO tareas(tarea, idcategoria, prioridad, idempresa, idusuario, requisito, creacion, inicio, entrega, recordatorio, anotaciones, estado) VALUES(:tarea, :idcategoria, :prioridad, :idempresa, :idusuario, :requisito, NOW(), :inicio, :entrega, :recordatorio, :anotaciones,'En Proceso')");	
	$stmt->bindValue(':tarea', $d["tarea"], PDO::PARAM_STR);
	$stmt->bindValue(':idcategoria', $d["categoria"], PDO::PARAM_INT);
	$stmt->bindValue(':prioridad', $d["prioridad"], PDO::PARAM_STR);
	$stmt->bindValue(':idempresa', $d["idempresa"], PDO::PARAM_INT);
	$stmt->bindValue(':idusuario', $d["idusuario"], PDO::PARAM_INT);
	$stmt->bindValue(':requisito', $d["requisito"], PDO::PARAM_INT);
	$stmt->bindValue(':inicio', $d["inicio"], PDO::PARAM_STR);
	$stmt->bindValue(':entrega', $d["entrega"], PDO::PARAM_STR);
	$stmt->bindValue(':recordatorio', $d["recordatorio"], PDO::PARAM_INT);
	...
	if (is_array($d["acciones"])) {
		...
		$value = '';
		$stmt = $db->prepare("INSERT INTO acciones_tareas(idtarea, idaccion, estado, idusuario) VALUES(:idtarea, :idaccion, 'Pendiente', :usuario)");
...
function actualizarTarea($d){
	global $db;
	$stmt = $db->prepare("UPDATE tareas SET idcategoria=:categoria, tarea=:tarea, prioridad=:prioridad, idempresa=:idempresa, idusuario=:idusuario, requisito=:requisito, inicio=:inicio, entrega=:entrega, culminacion=:culminacion, recordatorio=:recordatorio, anotaciones=:anotaciones, estado=:estado WHERE id=:id");	
	$stmt->bindValue(':tarea', $d["tarea"], PDO::PARAM_STR);
	...
	$stmt->bindValue(':inicio', $d["inicio"], PDO::PARAM_STR);
	$stmt->bindValue(':entrega', $d["entrega"], PDO::PARAM_STR);
	$stmt->bindValue(':culminacion', $d["culminacion"], PDO::PARAM_STR);
	$stmt->bindValue(':recordatorio', $d["recordatorio"], PDO::PARAM_INT);
	$stmt->bindValue(':anotaciones', $d["anotaciones"], PDO::PARAM_STR);
	$stmt->bindValue(':estado', $d["estado"], PDO::PARAM_STR);
	$stmt->bindValue(':id', $d["id"], PDO::PARAM_INT);
...
function usuariosQuery(){
	global $db;
	return $stmt = $db->query("SELECT idusuario,nombres from tblusuarios ORDER BY nombres")->fetchAll(PDO::FETCH_ASSOC);
}
?>