...
function CategoriasFormNuevo() {
    ...
    <form method="post" action="categorias.php?Accion=Guardar">
        ...
        <table width="100%">
            <tr>
                <td colspan="2" align="center" class="tituloFormulario"><b>NUEVA CATEGORIA MEMBRESIA</b></td>
            </tr>
        </table>
        <br/>
        <br/>
...
function CategoriasGuardar($nId, $nombre) {
    ...
    if ($nId <= 0) {
        $ca->prepareInsert("tblmb_categorias", "idcategoria,nombre");
        ...
        $ca->prepareUpdate("tblmb_categorias", "nombre", "idcategoria=:idcategoria");
        $ca->bindValue(":idcategoria", $nId, false);
        $ca->bindValue(":nombre", $nombre, true);
...
function CategoriasEliminar($nId) {
    $nConexion = Conectar();
    $ca = new DbQuery($nConexion);
    $ca->prepareDelete("tblmb_categorias", "idcategoria=:idcategoria");
    $ca->bindValue(":idcategoria", $nId, false);
...
function CategoriasFormEditar($nId) {
    $nConexion = Conectar();
    $ca = new DbQuery($nConexion);
    $ca->prepareSelect("tblmb_categorias", "*", "idcategoria=:idcategoria");
    $ca->bindValue(":idcategoria", $nId, false);
    $ca->exec();
    ...
    <form method="post" action="categorias.php?Accion=Guardar">
        ...
        <table width="100%">
            <tr>
                <td colspan="2" align="center" class="tituloFormulario"><b>EDITAR CATEGORIA MEMBRESIAS</b></td>
            </tr>
