...
<?php
  ...
  function CabezotesGuardar( $nId,$nombre,$color ) {
    ...
    $nConexion = Conectar();
    if ( $nId <= 0 ) {
      $sql = "INSERT INTO tblki_colores ( nombre,color ) 
      		VALUES ('{$nombre}','{$color}')";
	...
	exit;
      ...
      return;
    	...
    	$cTxtSQLUpdate = "UPDATE tblki_colores SET nombre='{$nombre}', color='{$color}'";
...
<?
  ...
  {
    $nConexion = Conectar();
    $reg = mysqli_fetch_object( mysqli_query($nConexion,"SELECT nombre FROM tblki_colores WHERE id ='$nId'") );
    $sql = "DELETE FROM tblki_colores WHERE id = $nId";
    $ra = mysqli_query($nConexion, $sql );
    if ( !$ra ) {
...
<?
  function CabezotesFormEditar( $nId ) {
    ...
    $nConexion = Conectar();
    $Resultado = mysqli_query($nConexion, "SELECT * FROM tblki_colores WHERE id = '$nId'" ) ;
    mysqli_close( $nConexion ) ;
