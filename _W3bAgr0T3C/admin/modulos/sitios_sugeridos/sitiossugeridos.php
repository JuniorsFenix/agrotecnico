<?
	...
	include("sitiossugeridos_funciones.php");
	include("../../herramientas/upload/uploaderFunction.php");
	include("../../vargenerales.php");
	if (!isset ($_GET["Accion"])) // Si no se envio la accion muestro la lista
	{
	header("Location: sitiossugeridos_listar.php");
	}
?>
<html>
	<head>
		<title>Administraci�n de Sitios Sugeridos</title>
  ...
  */
    switch($_GET["Accion"])
    {
      case  "Adicionar":
        ...
        break;
      case "Editar":
        SitiosFormEditar($_GET["Id"]);
        break;
      case "Eliminar":
        SitiosEliminar($_GET["Id"]);
        break;
      case "Guardar":
        ...
        if ( empty($Archivo) )
        {
          SitiosGuardar($_POST["txtId"],$_POST["txtNombre"],$_POST["txtDescripcion"],$_POST["txtWeb"],"",$_POST["optPublicar"]);
          exit;
        }
          ...
          foreach ($uploader_archivos_copiados as $imagen => $detalles)
          {
            ...
            # $imagen que almacena los nombres de las imagenes
      	    ...
      	    # mysqli_query($nConexion,"Insert into tabla (imagen,tipo,size) values (".$imagen.",".$detalles[0].",".$detalles[1].")",$coneccion);
						...
						SitiosGuardar($_POST["txtId"],$_POST["txtNombre"],$_POST["txtDescripcion"],$_POST["txtWeb"],$NomImagen,$_POST["optPublicar"]);
        }
        break;
      default:
        header("Location: sitiossugeridos_listar.php");
