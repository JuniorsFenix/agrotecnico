<?
  ...
  include("planes_funciones.php");
  include("../../herramientas/upload/uploaderFunction.php");
	include("../../vargenerales.php");
  if (!isset ($_GET["Accion"])) // Si no se envio la accion muestro la lista
  {
    header("Location: planes_listar.php");
  }
?>
<html>
	<head>
		<title>Administraci�n de Planes</title>
  ...
  */
    switch($_GET["Accion"])
    {
      case  "Adicionar":
        ...
        break;
      case "Editar":
        PlanesFormEditar($_GET["Id"]);
        break;
      case "Eliminar":
        PlanesEliminar($_GET["Id"]);
        break;
      case "Guardar":
        ...
        if ( empty($Archivo) )
        {
          PlanesGuardar($_POST["txtId"],$_POST["txtTitulo"],"",$_POST["txtDescripcion"],$_POST["date"],$_POST["optPublicar"],$_POST["cboSede"]);
          exit;
        }
          ...
          foreach ($uploader_archivos_copiados as $imagen => $detalles)
          {
            ...
            # $imagen que almacena los nombres de las imagenes
      	    ...
      	    # mysqli_query($nConexion,"Insert into tabla (imagen,tipo,size) values (".$imagen.",".$detalles[0].",".$detalles[1].")",$coneccion);
						...
						PlanesGuardar($_POST["txtId"],$_POST["txtTitulo"],$NomImagen,$_POST["txtDescripcion"],$_POST["date"],$_POST["optPublicar"],$_POST["cboSede"]);
        }
        break;
      default:
        header("Location: planes_listar.php");
