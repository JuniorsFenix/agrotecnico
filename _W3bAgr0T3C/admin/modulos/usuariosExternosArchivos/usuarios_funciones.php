...
?>
<?php
function UsuariosGuardar( $nId,$cDescripcion ) {
    $IdCiudad = $_SESSION["IdCiudad"];
    $nConexion = Conectar();
    if ( $nId <= 0 ) {
...
function UsuariosEliminar( $nId ) {
    $nConexion = Conectar();
    $reg = mysqli_fetch_object( mysqli_query($nConexion,"SELECT * FROM tblusuarios_archivos WHERE idarchivo ='$nId'") );
    mysqli_query($nConexion, "DELETE FROM tblusuarios_archivos WHERE idarchivo ='$nId'" );
    Log_System( "USUARIOS_ARCHIVOS" , "ELIMINA" , "USUARIOS_ARCHIVOS: " . $reg->idarchivo );
    mysqli_close( $nConexion );
	
	@ unlink("../../../fotos/usuariosarchivos/{$reg->idarchivo}");
    
	if ( $reg->tipo == "VIDEOS" ) {
	  $path = "../../../fotos/usuariosarchivos/{$reg->idarchivo}";
	  unlink("{$path}.flv");
	  unlink("{$path}.jpg");
	}
