<?
  ...
  if ( isset($_POST['filtro_usuario'])) {

	if ( $_POST['filtro_usuario'] != "Todos" ) {

	  $filtro = "and usuario = '{$_POST['filtro_usuario']}'";

	}

	if ( $_POST['filtro_archivo'] != "Todos" ) {

	  $filtro .= " and tipo = '{$_POST['filtro_archivo']}'";
      ...
      <tr>
        ...
        <td colspan="12" align="center" class="tituloFormulario">
          ...
          <br>
			...
			<p align="left">Usuario: <select name="filtro_usuario" id="filtro_usuario" align="left">

		  <option value="Todos" SELECTED>Todos</option>

			<?php while ( $row=mysqli_fetch_array($raUsuarios)):?>

		  <option value="<?=$row['usuario'];?>" <?=$_POST['filtro_usuario']==$row['usuario']?"SELECTED":"";?>><?=$row['usuario'];?></option>

		  <?php endwhile;?>

		</select>

		Tipo archivo: <select name="filtro_archivo" id="filtro_archivo" align="left">

		  <option value="Todos" SELECTED>Todos</option>

			<?php while ( $row=mysqli_fetch_array($raTipos)):?>

		  <option value="<?=$row['tipo'];?>" <?=$_POST['filtro_archivo']==$row['tipo']?"SELECTED":"";?>><?=$row['tipo'];?></option>

		  <?php endwhile;?>

		</select><input type="submit" name="filtrar" value="Filtrar">
