...
?>
    ...
    <form action="imagenes_categorias.php?Accion=Guardar" method="post" enctype="multipart/form-data">
      ...
      <table width="100%">
        ...
        <tr>
          <td colspan="2" class="nuevo">
            ...
            <a href="imagenes_categorias_listar.php"><img src="../../image/cancelar.gif" border="0" alt="Cancelar y Regresar."></a>
          </td>
        </tr>
      </table>
</form>
  ...
  function CategoriasGuardar( $nIdPadreActual,$nId,$nIdPadre,$cCategoria,$cDescripcion,$cImagen )
  {
    ...
    if ( $nId <= 0 ) // Nuevo Registro
    {
      ...
      //mysqli_query($nConexion,"INSERT INTO tblcategoriasproductos ( idpadre,categoria,descripcion,imagen ) VALUES ( '$nIdPadre','$cCategoria','$cDescripcion','$cImagen' )");
			if ( $nIdPadre == 0 )
			{
				...
				$rsContador						= mysqli_query($nConexion,"SELECT MAX( CAST(idcategoria AS UNSIGNED) ) AS contador FROM tblcategoriasimagenes WHERE (idpadre = '$idNuevoPadre') AND (idciudad = $IdCiudad)");
				...
				mysqli_query($nConexion,"INSERT INTO tblcategoriasimagenes ( idcategoria,idpadre,categoria,descripcion,imagen,idciudad ) VALUES ('$NuevoIdCategoria','0', '$cCategoria','$cDescripcion','$cImagen',$IdCiudad)");
				Log_System( "IMAGENES CATEGORIAS" , "NUEVO" , "CATEGORIA: " . $cCategoria  );
			} //FIN if ( $nIdPadre == 0 )
			else
			{
				$rsContador = mysqli_query($nConexion,"SELECT COUNT(*)+1 AS contador FROM tblcategoriasimagenes WHERE (idpadre = '$nIdPadre') AND (idciudad = $IdCiudad)");
				...
				mysqli_query($nConexion,"INSERT INTO tblcategoriasimagenes (idcategoria,idpadre,categoria,descripcion,imagen,idciudad) VALUES ('$cCodCategoria','$nIdPadre','$cCategoria','$cDescripcion','$cImagen',$IdCiudad)");
				Log_System( "IMAGENES CATEGORIAS" , "NUEVO" , "CATEGORIA: " . $cCategoria  );
			}
      mysqli_close($nConexion);
      Mensaje( "El registro ha sido almacenado correctamente.", "imagenes_categorias_listar.php" ) ;
      exit;
    }
    else // Actualizar Registro Existente
    {
			...
			if ( $nIdPadreActual != $nIdPadre )
			{
				...
				$rsContador						= mysqli_query($nConexion,"SELECT COUNT(*) AS contador FROM tblcategoriasimagenes WHERE (idpadre = '$idNuevoPadre') AND (idciudad = $IdCiudad)");
				$RegistrosRSContador	= mysqli_fetch_object($rsContador);
				$Contador							= $RegistrosRSContador->contador;
				if ( $Contador == null )
				{
				...
				if ( !empty( $cImagen ) )
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasimagenes SET idcategoria = '$NuevoIdCategoria', idpadre = '$idNuevoPadre', categoria = '$cCategoria', descripcion = '$cDescripcion', imagen = '$cImagen' WHERE idcategoria = '$nId' AND ( idciudad = $IdCiudad )" );
					Log_System( "IMAGENES CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				else
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasimagenes SET idcategoria = '$NuevoIdCategoria', idpadre = '$idNuevoPadre', categoria = '$cCategoria', descripcion = '$cDescripcion' WHERE idcategoria = '$nId' AND ( idciudad = $IdCiudad )" );
					Log_System( "IMAGENES CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				// Actualizo las categorias de los servicios que pertenecian al anterior codigo categoria
				mysqli_query($nConexion,"UPDATE tblimagenes SET idcategoria = '$NuevoIdCategoria' WHERE idcategoria =  '$nId' AND ( idciudad = $IdCiudad )");
			}
			else // No cambio la categoria actualizo los demas datos
			{
				if ( !empty( $cImagen ) )
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasimagenes SET categoria = '$cCategoria', descripcion = '$cDescripcion', imagen = '$cImagen' WHERE idcategoria = '$nId' AND ( idciudad = $IdCiudad )" );
					Log_System( "IMAGENES CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
				else
				{
					mysqli_query($nConexion, "UPDATE tblcategoriasimagenes SET categoria = '$cCategoria', descripcion = '$cDescripcion' WHERE idcategoria = '$nId' AND ( idciudad = $IdCiudad )" ) or die (mysqli_error($nConexion));
					Log_System( "IMAGENES CATEGORIAS" , "EDITA" , "CATEGORIA: " . $cCategoria  );
				}
			}
      mysqli_close( $nConexion );
      Mensaje( "El registro ha sido actualizado correctamente.", "imagenes_categorias_listar.php" ) ;
      exit;
    }
  } // FIN: function 
  ###############################################################################
...
<?
  ...
  function CategoriasEliminar( $nId )
  {
    $IdCiudad = $_SESSION["IdCiudad"];
    $nConexion = Conectar();
    $reg = mysqli_fetch_object( mysqli_query($nConexion,"SELECT categoria FROM tblcategoriasimagenes WHERE idcategoria ='$nId' AND ( idciudad = $IdCiudad )") );
    mysqli_query($nConexion, "DELETE FROM tblcategoriasimagenes WHERE idcategoria ='$nId' AND ( idciudad = $IdCiudad )" );
    Log_System( "IMAGENES CATEGORIAS" , "ELIMINA" , "CATEGORIA: " . $reg->categoria  );
    $sql = "delete from tblimagenes where idcategoria={$nId}";

    mysqli_query($nConexion,$sql);

    mysqli_close( $nConexion );
    Mensaje( "El registro ha sido eliminado correctamente.","imagenes_categorias_listar.php" );
    exit();
...
<?
  ...
  function CategoriasFormEditar( $nId )
  {
    ...
    $nConexion    = Conectar();
    $Resultado    = mysqli_query($nConexion, "SELECT tblcategoriasimagenes.idcategoria, tblcategoriasimagenes.idpadre, tblcategoriasimagenes.categoria, tblcategoriasimagenes.descripcion, tblcategoriasimagenes.imagen, COUNT(tblimagenes.idimagen) AS timagenes FROM tblcategoriasimagenes LEFT OUTER JOIN tblimagenes ON tblcategoriasimagenes.idcategoria = tblimagenes.idimagen WHERE tblcategoriasimagenes.idcategoria = '$nId' AND ( tblcategoriasimagenes.idciudad = $IdCiudad ) GROUP BY tblcategoriasimagenes.idcategoria, tblcategoriasimagenes.idpadre, tblcategoriasimagenes.categoria, tblcategoriasimagenes.descripcion, tblcategoriasimagenes.imagen" ) ;
    $ResultadoSubCat = mysqli_query($nConexion, "SELECT COUNT(tblcategoriasimagenes.idcategoria) AS tsubcategorias FROM tblcategoriasimagenes WHERE (idpadre = '$nId') AND idciudad = $IdCiudad" );
		mysqli_close( $nConexion ) ;
...
?>
    ...
    <form method="post" action="imagenes_categorias.php?Accion=Guardar" enctype="multipart/form-data">
      ...
      <table width="100%">
        <tr>
          <td colspan="2" align="center" class="tituloFormulario">
            ...
            $nConexion = Conectar();
            $ResultadoCat = mysqli_query($nConexion, "SELECT * FROM tblcategoriaimagenes WHERE icdiudad = $IdCiudad ORDER BY idcategoria" );
            mysqli_close($nConexion);
        ...
        <tr>
        		...
        		<td class="contenidoNombres">
						<?
							...
							{
								?><img src="<? echo $cRutaVerImgCategorias_Galeria . $Registro["imagen"]; ?>"><?
							}
						?>
						</td>
       		</tr>
        <tr>
          ...
          <?
            $cMSGEliminar = "";
						$cEliminable  = "SI";
            if ( (($Registro[ "timagenes" ] > 0) or ($RegistroSubCat[ "tsubcategorias" ] > 0)) )
            {
							$cEliminable  = "NO";
							if ( $Registro[ "idpadre" ] == 0  )
							{
          ...
          <td class="contenidoNombres"><? echo $RegistroSubCat[ "tsubcategorias" ] . " SubCategor�a(s).<br>" ;?></td>
				...
				<tr>
					<td>&nbsp;</td>
					<td class="contenidoNombres"><? echo $Registro[ "timagenes" ] . " Imagen(es).<br>" ?></td>
				</tr>
				<?	
				if ( $cEliminable == "NO" )
				{
        ...
        <tr>
          <td colspan="2" class="nuevo">
            <input type="image" src="../../image/guardar.gif" alt="Guardar Registro.">
            <a href="imagenes_categorias.php?Accion=Eliminar&Id=<? echo $nId ;?>">
                <img src="../../image/eliminar.gif" border="0" alt="Eliminar Registro." onClick="javascript: return confirm('�Esta seguro que desea eliminar este registro y todas sus imagenes asociadas?')">
            </a>
            <a href="imagenes_categorias_listar.php"><img src="../../image/cancelar.gif" border="0" alt="Cancelar y Regresar."></a>
          </td>
        </tr>
